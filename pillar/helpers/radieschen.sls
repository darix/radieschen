...
{%- macro sentinel_default_acls(sentinel_username="sentinel", sentinel_key) %}
    - 'default off nopass'
...
{%- macro sentinel_auth_config(sentinel_username="sentinel", sentinel_key) %}
    - 'sentinel sentinel-user {{ sentinel_username }}'
    - 'sentinel sentinel-pass {{ sentinel_key | gopass(quoted=False) }}'
{%- endmacro %}

{%- macro redis_sentinel_auth_config(sentinel_username="sentinel", sentinel_key) %}
...
{{ redis_default_settings(non_tls_port=0, bind_ips=["0.0.0.0"], replication_user = 'replicator') }}
        tls-port: {{ tls_port }}
        ...
        # enforce client certificate can be reduced to optional if you want tls without clients
